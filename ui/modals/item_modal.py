...
class ItemModal(QDialog):
    def __init__(self, parent: QWidget | None = None, *, item: Item | None = None) -> None:
        ...
        form.addRow("Mã kho", self._category)
    ...
    def item(self) -> Item:
        ...
        item.category = ItemCategory(self._category.currentData())
        ...
        # Account follows the category by default (152/153/155/156) now that the
        # manual "Tài khoản kế toán" field has been removed from the form.
        item.account_code = item.category.value
