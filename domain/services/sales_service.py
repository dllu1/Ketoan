...
class SalesService(DocumentService):
    ...
    def _apply_posting(self, invoice: Invoice) -> None:
        ...
        # theo dòng → đầu chứng từ → mặc định. TK kho (line.account_code) chỉ để
        # xuất kho; GIÁ VỐN KHÔNG ghi ở đây — nó được kết chuyển cuối kỳ theo đơn
        # giá bình quân cuối kỳ (CogsService), để lương công nhân về trễ vẫn vào
        # đủ giá thành trước khi tính giá vốn.
        debit_by_account: dict[str, Decimal] = {}
        revenue_by_account: dict[str, Decimal] = {}
        for line in invoice.lines:
            if not line.item_code or line.quantity <= 0:
                continue
            self._inventory.record_out(
